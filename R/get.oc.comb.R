...
#'                if MTD.contour=TRUE; Otherwise, a scalar specifying the total number of cohorts for
#'                the trial.
...
get.oc.comb <- function(target, p.true, ncohort, cohortsize, n.earlystop=NULL, startdose=c(1,1), p.saf=0.6*target, p.tox=1.4*target,
                       cutoff.eli=0.95, extrasafe=FALSE, offset=0.05, ntrial=1000, MTD.contour=FALSE)
{
  ...
  get.oc.comb.boin <- function(target, p.true, ncohort, cohortsize, n.earlystop = 100, startdose = c(1,1), p.saf = 0.6*target,
             p.tox = 1.4*target, cutoff.eli = 0.95, extrasafe = FALSE, offset = 0.05, ntrial = 1000)
  {
      ...
      for (trial in 1:ntrial)
      {
        ...
        for (pp in 1:ncohort)
        {
          ...
          if (y[d[1],d[2]] <= b.e[nc])
          {
            ...
            {
              k = which(pr_H0 == max(pr_H0))[as.integer(runif(1) * length(which(pr_H0 ==
                                                                                  max(pr_H0))) + 1)];
        ...
        {
          selcomb = select.mtd.comb(
            target, n, y, cutoff.eli, extrasafe, offset, print = FALSE, MTD.contour =
              FALSE
          );
      ...
      for (i in 1:dim(p.true)[1])
        for (j in 1:dim(p.true)[2]) {
          {
            selpercent[i,j] = sum(dselect[,1] == i &
                                    dselect[,2] == j) / ntrial * 100;
          }
        }
      ...
      if (JJ <= KK) {
        ...
        for (i in 1:dim(p.true)[1]) {
          cat(formatC(
            p.true[i,], digits = 2, format = "f", width = 5
          ), sep = "  ", "\n");
        }
        cat("\n");
        ...
        for (i in 1:dim(p.true)[1]) {
          cat(formatC(
            selpercent[i,], digits = 2, format = "f", width = 5
          ), sep = "  ", "\n");
        }
        cat("\n");
        ...
        for (i in 1:dim(p.true)[1]) {
          cat(formatC(
            apply(N,c(1,2),mean)[i,], digits = 2, format = "f", width = 5
          ), sep = "  ", "\n");
        }
        cat("\n");
        ...
        for (i in 1:dim(p.true)[1]) {
          cat(formatC(
            apply(Y,c(1,2),mean)[i,], digits = 2, format = "f", width = 5
          ), sep = "  ", "\n");
        }
        cat("\n");
        cat(
          "average number of toxicities:", formatC(sum(Y) / ntrial, digits = 1, format =
                                                     "f"), "\n"
        );
        cat("average number of patients:", formatC(sum(N) / ntrial, digits =
                                                     1, format = "f"), "\n");
        cat("selection percentage of MTD:", formatC(sum(selpercent[which(p.true ==
                                                                           target, arr.ind = TRUE)]), digits = 1, format = "f"), "\n");
        cat(
          "percentage of patients treated at MTD:", formatC(
            sum(nptsdose[which(p.true == target, arr.ind = TRUE)]) / npts * 100, digits =
              1, format = "f"
          ), "\n"
        );
        cat(
          "percentage of early stopping due to toxicity:", formatC(
            100 - sum(selpercent), digits = 2, format = "f"
          ), "\n"
        );
      }
      else{
        ...
        cat("\n");
        cat("average number of toxicities:", formatC(t(sum(Y)) / ntrial, digits =
                                                       1, format = "f"), "\n");
        cat("average number of patients:", formatC(t(sum(N)) / ntrial, digits =
                                                     1, format = "f"), "\n");
        cat("selection percentage of MTD:", formatC(sum(selpercent[which(p.true ==
                                                                           target, arr.ind = TRUE)]), digits = 1, format = "f"), "\n");
        cat(
  ...
  waterfall.subtrial.mtd <- function(target, npts, ntox, cutoff.eli=0.95, extrasafe=FALSE, offset=0.05)
  {
      ...
      temp = get.boundary(target, ncohort=150, cohortsize=1, cutoff.eli=cutoff.eli, extrasafe=extrasafe, print=FALSE);
      b.e = temp[2,];   # escalation boundary
  ...
  startdose=1, p.saf=0.6*target, p.tox=1.4*target, cutoff.eli=0.95, extrasafe=FALSE, offset=0.05, totaln)
  {
             ...
             p.saf=0.6*target, p.tox=1.4*target, extrasafe=FALSE, offset=0.05, ntrial=1000)
  {
      ...
      for (trial in 1:ntrial) {
        ...
        while (totaln < sum(ncohort) * cohortsize) {
          subtrial = waterfall.subtrial(target, p.true = p.true,  dosespace = dosespace, npts = npts, ntox = ntox, elimi =
            ...
            p.saf=p.saf, p.tox=p.tox, cutoff.eli=cutoff.eli, extrasafe=extrasafe, offset=offset, totaln = totaln)
      ...
      if (JJ > KK) {
        ...
        for (i in 1:dim(p.true)[2])
          cat(formatC(
            nptsdose[,i] / ntrial, digits = 2, format = "f", width = 5
          ), sep = "  ", "\n");
        cat("\n");
        ...
        for (i in 1:dim(p.true)[2])
          cat(formatC(
            ntoxdose[,i] / ntrial, digits = 2, format = "f", width = 5
          ), sep = "  ", "\n");
        cat("\n");
        cat("average number of toxicities:", formatC(
          sum(ntoxdose / ntrial), digits = 1, format = "f"
        ), "\n");
        cat("average number of patients:", formatC(
          sum(nptsdose / ntrial), digits = 1, format = "f"
        ), "\n");
        if (length(at.contour) > 0) {
          cat(
            "percentage of patients treated at MTD contour:", paste(round(
              100 * sum(nptsdose[at.contour]) / sum(nptsdose),1
            ),'%',sep = ''), "\n"
          );
          cat(
            "percentage of patients treated above MTD contour:", paste(round(
              100 * sum(nptsdose[greater.than.contour]) / sum(nptsdose),1
            ),'%',sep = ''), "\n"
          );
          cat(
            "percentage of patients treated below MTD contour:", paste(round(
              100 * sum(nptsdose[less.than.contour]) / sum(nptsdose),1
            ),'%',sep = ''), "\n"
          );
          cat(
            "percentage of correct selection of the MTD contour:", paste(round(100 *
  ...
  if (MTD.contour == FALSE) {
   ...
   if (((JJ * KK) > 4) & (sum(ncohort) <= 8))  {
    ...
    get.oc.comb.boin(target, p.true, ncohort = sum(ncohort), cohortsize, n.earlystop, startdose, p.saf, p.tox, cutoff.eli,
    extrasafe, offset, ntrial)
  }
  if (MTD.contour == TRUE) {
  ...
  if (((JJ * KK) > 4) & (sum(ncohort) <= 8))  {
    ...
    get.oc.comb.waterfall(p.true, target, ncohort, cohortsize, n.earlystop = n.earlystop, cutoff.eli, p.saf, p.tox, extrasafe, offset, ntrial)
